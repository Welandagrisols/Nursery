...
export function PriceListTab() {
  ...
  const { nurseryName, logoUrl, phone: nurseryPhone, location: nurseryLocation } = useNursery()
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div ref={printRef} className="space-y-6">
          ...
          <div className="hidden print:block text-center mb-4 border-b pb-4 space-y-1">
            {logoUrl && (
            ...
            {(nurseryLocation || nurseryPhone) && (
              <p className="text-gray-500 text-sm">{[nurseryLocation, nurseryPhone].filter(Boolean).join(" · ")}</p>
            )}
            <p className="text-gray-400 text-xs">Price List — {new Date().toLocaleDateString("en-KE", { day: "numeric", month: "long", year: "numeric" })}</p>
          </div>
